...
y,po,u1,u2,u3,u4 = symbols('y,po,u1,u2,u3,u4')

        

def easy_test():
    p = y(u4,u2,u2+u3,u3+u4)+y(u4,u1,u1+u2,u2+u4) + y(u3,u1,u1+u2,u2+u3) + y(u4,u1,u1+u3,u3+u4)
    for s1 in (0,1):
        p1 = p.subs(u1,s1)
        for s2 in (0,1):
            p2 = p1.subs(u2,s2)
...
def easy_test_p2_1():
    p = po(u4,u2,u3+u4,u2+u3,2)+po(u4,u1,u3+u4,u1+u3,2) + y(u4,u1,u1+u2,u2+u4) + y(u3,u1,u1+u2,u2+u3)+(u1+u2)*(u3+u4+1)
    for s1 in (0,1):
        p1 = p.subs(u1,s1)
        for s2 in (0,1):
            p2 = p1.subs(u2,s2)
